...
const Images = (props) => {
  ...
  const uQ = useQuery(USER_QUERY, {
    variables: { currentUserId },
  })

  const { error, data } = currentUserId ? uQ : dummyObject
              ...
              >
                <div className="header-nav-items" style={{ marginLeft: 20 }}>
                  {image.user?.profilePicUrl ? (
                    <img src={image.user?.profilePicUrl} alt={image.url} />
                  ) : (
              ...
              <Card.Body id="cardBody" bsPrefix="div">
                <section id="icons-and-comment-form">
                  <div className="flex">
                    ...
                    <button
                      className="block like-and-comment-icons"
                      onClick={() => handleShow(image.id)}
                    >
                      <Chat size={20} />
                  ...
                  <button onClick={() => handleShow(image.id)}>
                    ...
                    <p id="created-at" className="rc-font-size">
                      {image?.createdAt}
                    </p>
                  </button>
                </section>
