...
const EditUserInfoForm = (props) => {
  ...
  const handleFirebaseUpload = (e) => {
    ...
    if (profilePicAsFile === '') {
      console.error(
        `not an image, the image file is a ${typeof profilePicAsFile}`
      )
    }
    const uploadTask = storage
      .ref(`/profile-pics/${profilePicAsFile.name}`)
      .put(profilePicAsFile)
    uploadTask.on(
      'state_changed',
      (snapShot) => {
    ...
    <div className="rw-form-wrapper">
      <Form onSubmit={onSubmit} error={props.error}>
        ...
        {showInput && !profilePicUrl && (
          <input type="file" onChange={handleProfilePicAsFile} />
        )}
        {showUpload && (
          <div style={{ marginTop: '50px', maxWidth: '25%' }}>
            <button
              className="rw-button-group rw-button rw-button-green"
