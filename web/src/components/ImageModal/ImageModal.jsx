...
const ImageModal = (props) => {
  ...
  const currentUserLikesThis = image?.likedBy?.some(
    (item) => item.id === currentUserId
  )
  const userIsValidAndOwnsImage = Boolean(
    ...
    <>
      ...
      >
        <Modal.Body>
          <Container>
            <Row>
              <Col md={8} id="left-column">
                <div id="modal-image-container">
                  <img
                    alt={image?.url}
                    src={image?.url}
                    onDoubleClick={() =>
                      !missingData && handleLikeRequests(currentUserLikesThis)
                    }
                  />
                </div>
              </Col>
              <Col md={4} id="right-column">
                <article>
                  <section id="miniProfPicAndHandle">
                    {image?.user.profilePicUrl ? (
                      <img
                        src={image?.user.profilePicUrl}
                        alt={image?.user.profilePicUrl}
                      />
                    ) : (
                  ...
                  {image?.title && (
                    <section id="caption">
                      <p
                        className="rc-font-size allow-newline"
                        style={{ marginTop: 10 }}
                      >
                        {image?.title}
                      </p>
                  ...
                  <section id="comments-box">
                    {image?.comments &&
                      image?.comments.map((modalComment) => (
                        <Comment
                ...
                <article id="article-of-icons-and-comment-form">
                    ...
                    <div className="flex">
                      <div className="block like-and-comment-icons">
                        {currentUserLikesThis ? (
                          <button
                            onClick={() =>
                              handleLikeRequests(currentUserLikesThis)
                            }
